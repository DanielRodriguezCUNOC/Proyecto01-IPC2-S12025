<%@ page import="com.happycomputer.modelos.VentaModelo" %>
...
<div class="container mx-auto p-4">
    <h1 class="text-2xl font-bold mb-4">Consultar Ventas</h1>
    <form action="/SvConsultaCompraCliente" method="get" class="bg-white p-6 rounded-lg shadow-lg">
        <div class="mb-4">
    ...
    <table class="w-full bg-white rounded-lg shadow-lg mt-4">
        ...
        <tbody>
        <%
            List<VentaModelo> ventas = (List<VentaModelo>) request.getAttribute("ventas");
            if (ventas != null) {
                for (VentaModelo venta : ventas) {
        %>
        <tr>
            <td class="p-2"><%= venta.getId() %>
            </td>
            <td class="p-2"><%= venta.getFechaVenta() %>
            </td>
            <td class="p-2"><%= venta.getTotalVenta() %>
            </td>
        </tr>
        <%
                }
